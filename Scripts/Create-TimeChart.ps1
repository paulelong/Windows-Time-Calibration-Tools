...
if (-not (test-path $GraphDataBackupDir))  { md $GraphDataBackupDir }
    ...
    {
        echo ("Missing .out data file(s) for System Under Test " + $Name + ". Could not find " + $ServerOut_IP) 
        return $false
    }
    ...
    #Remove file first since we append data.
    if (test-path $ServerDif_IP)  { del  $ServerDif_IP }
    if (test-path $ServerPlot_IP)  { del  $ServerPlot_IP }
    ...
    if((dir $ServerDif_IP).Length -gt 0)
    {
        ...
        echo 'set ytics add ("+1000us/1ms" 1000) add ("-1000us/1ms" -1000)' | Out-file $PlotGP_IP -Encoding ascii -Append
